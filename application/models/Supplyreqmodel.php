...
"left join clients on supply_requests.clientno=clients.client_no where department='".$dept."' and YEAR(supply_requests.csrdate)=YEAR(CURDATE()) order by supply_requests.csrno desc
");   
          $data = $query->result();
          }else{
                $query = $this->db->query("select supply_requests.clientno,clients.clientname,supply_requests.csrno, supply_requests.description,
                supply_requests.csrdate,supply_requests.podate, supply_requests.ponum,supply_requests.currency,supply_requests.csrvalue,
                supply_requests.soldby,supply_requests.num,supply_requests.invoice_no  from supply_requests
                left join clients on supply_requests.clientno=clients.client_no where YEAR(supply_requests.csrdate)=YEAR(CURDATE()) order by supply_requests.csrno desc");   
                 $data = $query->result();
            }
        ...
        function getcsritems($csrno){
              $csritems = $this->db->query("select partno, description,qty,unitcost,id,vat,unitbp,margin,markuprate from csritems where csrno='".$csrno."'")->result();
              return $csritems;
        }
