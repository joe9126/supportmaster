...
class Supplyrequests extends MY_Sessions{
   ...
   /* function checkcsrnum(){
    ...
    function createcsr(){
        ...
        if($status<1){
          $csrdata = array("clientno"=>$clientid,"csrno"=>$csrnum,"description"=>$description,"csrdate"=>$csrdate,
              ...
              "fin_approval"=>$finmanager,"dir_approval"=>$dirmanager,"totalbp"=>$grandtotalBP,"margin"=>$marginrate,"markuprate"=>$marginrate,"department"=>$dept); 
    ...
    //  department:department,csrnum:csrnum,partno:partno,descript:descript,qty:qty,unitcost:unitcost,vat:vat  
      ...
      if($itemnumstatus<1){
       $data = array("CSRNO"=>$csrnum,"PARTNO"=>$partno,"DESCRIPTION"=>$descript,"QTY"=>$qty,"UNITCOST"=>$unitSP,"MARGIN"=>$margin,"UNITBP"=>$unitBP,"VAT"=>$vat);
      $status = $this->supplyreqmodel->additems($data);
        }else
            {
             $data = array("CSRNO"=>$csrnum,"PARTNO"=>$partno,"DESCRIPTION"=>$descript,"QTY"=>$qty,"UNITCOST"=>$unitBP,"VAT"=>$vat);
